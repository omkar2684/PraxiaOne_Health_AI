...
class _AssistantScreenV2State extends State<AssistantScreenV2> {
  ...
  final List<Map<String, dynamic>> _messages = [
    {
      'text': 'Hello! I am Praxia, your Health Assistant. How can I help you with your vitals or reports today?',
      'isUser': false,
      ...
      setState(() {
        ...
        for (var msg in history) {
          _messages.add(<String, dynamic>{
            ...
            'results': msg['results'] ?? <String, dynamic>{},
          });
        }
        _isLoading = false;
      });
  ...
  void _sendMessage(String text) async {
    ...
    setState(() {
      if (currentText.isNotEmpty || fileToSend != null) {
        _messages.add({
          'text': currentText, 
          'isUser': true,
          'attachedFilePath': fileToSend?.path,
          'attachedFileName': fileToSend?.name,
          'attachedFileExt': fileToSend?.extension,
        });
      }
      _isLoading = true;
      _selectedFile = null;
    ...
    try {
      final resp = await ApiService.chat(finalMessage, docId: uploadedDocId);
      if (mounted) {
        setState(() {
          _messages.add(<String, dynamic>{
            'text': resp['reply'] ?? 'No response',
            'results': resp['results'] ?? <String, dynamic>{}, // Multi-model results
            'isUser': false,
            'hasAudio': false,
          });
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Ask Praxia', style: TextStyle(color: AppColors.accent, fontWeight: FontWeight.w900, fontSize: 18)),
