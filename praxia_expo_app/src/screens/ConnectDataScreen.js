        ...
        <View style={styles.timelineCont}>
          ...
          {documentUploaded && isAiThinking && (
            <View style={styles.aiThinkingCard}>
              ...
              <Text style={styles.aiThinkingTitle}>Praxia AI is analyzing...</Text>
          ...
          {documentUploaded && !isAiThinking && riskFactors && (
            <View style={[styles.riskCard, { borderColor: isStable ? '#DCFCE7' : '#F1F5F9' }]}>
              ...
              {riskFactors.raw_biomarkers && (
                <TouchableOpacity 
                  style={{ backgroundColor: '#2563EB', padding: 14, borderRadius: 10, marginTop: 15, alignItems: 'center' }}
                  onPress={() => navigation.navigate('LabResults', { biomarkers: riskFactors.raw_biomarkers })}
                >
                  <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 14 }}>View Full Lab Results</Text>
                </TouchableOpacity>
              )}
            </View>
          )}
