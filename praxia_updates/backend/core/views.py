...
class TrackProgressView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            "actions": actions,
            "insights": [
                {"icon": "trending_up", "text": f"Your activity is up 22% with {wearable_data['steps']} daily steps", "color": "success"},
                {"icon": "bedtime", "text": f"Consistent sleep detected ({wearable_data['sleep_duration']}h avg)", "color": "primary"}
            ],
            "projection": {
                "text": "You are on track to improve your health metrics.",
                "subtext": "Based on your current adherence pattern",
                "biomarkers": [
                    {"name": "Fasting Glucose", "improvement": "-8%", "from_to": "From 102 to ~94 mg/dL", "trend": "down"},
                    {"name": "LDL Cholesterol", "improvement": "-5%", "from_to": "From 134 to ~127 mg/dL", "trend": "down"}
                ]
            },
            "re_test": {
                "days_left": 18,
